...
import com.github.flotskiy.bookshop.model.dto.user.UserBooksData;
...
import com.github.flotskiy.bookshop.service.Book2UserService;
...
public class BookstoreUserDetailsService implements UserDetailsService {
    ...
    public UserBooksData createUserBooksData(Integer userId) {
        ...
        for (Map.Entry<String, Integer> entry : bookToUserTypeMap.entrySet()) {
            switch (entry.getKey()) {
                case "KEPT":
                    userBooksData.setKept(getBookDtoListFromBook2UserList(userId, entry.getValue()));
                    break;
                case "CART":
                    userBooksData.setCart(getBookDtoListFromBook2UserList(userId, entry.getValue()));
                    break;
                case "PAID":
                    userBooksData.setPaid(getBookDtoListFromBook2UserList(userId, entry.getValue()));
                    break;
                case "ARCHIVED":
                    userBooksData.setArchived(getBookDtoListFromBook2UserList(userId, entry.getValue()));
                    break;
                default:
                    throw new WrongBookStatusException("Unknown book status: " + entry.getKey());
            }
        }
