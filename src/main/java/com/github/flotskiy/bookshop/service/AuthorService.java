...
import com.github.flotskiy.bookshop.model.entity.book.links.Book2AuthorEntity;
import com.github.flotskiy.bookshop.repository.AuthorRepository;
...
public class AuthorService {
    ...
    public Map<String, List<AuthorDto>> getAuthorsGroupedMap() {
        ...
        Map<String, List<AuthorDto>> result = authorDtoList.stream()
    ...
    public List<AuthorEntity> sortAuthorEntitiesBySortIndex(Set<AuthorEntity> authorEntities, int bookId) {
        ...
        for (Book2AuthorEntity book2Author : book2AuthorEntityList) {
            Optional<AuthorEntity> authorEntityOptional = authorEntities.stream()
                    .filter(authorEntity -> authorEntity.getId() == book2Author.getAuthorId())
                    .findFirst();
            AuthorEntity author = authorEntityOptional.orElseThrow(EntityNotFoundException::new);
            result.add(author);
        }
        return result;
    }
