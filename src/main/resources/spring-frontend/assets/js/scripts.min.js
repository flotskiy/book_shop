    ...
    $(function () {
        ...
        var Card = function () {
            ...
            var bookTemplate = function (book) {
                var author = book.authors.includes('\,') ? book.authors.split('\,', 1)[0] + others : book.authors;
                return '<div class="Card">' +
                       '   <div class="Card-picture">' +
                            '<a href="/books/' + book.slug + '">' +
                            '       <img src="' + book.image + '">\n' +
                    (book.discount ? '<div class="Card-sale">' + book.discount + discount + '</div>' : '') +
                    (book.isBestseller ? '<div class="Card-ribbon"><span class="Card-ribbonText">' + bestseller + '</span></div>' : '') +
                    ...
                    '                <div class="Card-cost">' +
                    (book.discount ? '<span class="Card-priceOld">₽' + book.price + '</span>\n' : '') + '                       <span class="Card-price">₽' + book.discountPrice + '</span>\n' + '                </div>\n' + '              </div>\n' + '            </div>';
            }
            return {
                init: function () {
                    $('[data-refreshshow]').on('click change', function (e) {
                        ...
                        function getData(address, data, type) {
                            ...
                            $.ajax({
                                ...
                                complete: function (result) {
                                    if (result.status === 200) {
                                        if (result.responseJSON.count < $this.data('refreshlimit')) {
                                            document.getElementById("butmore").remove();
                                        }
                                        var data = '';
                                        result.responseJSON.books.forEach(function (book) {
            ...
            return {
                init: function () {
                    ...
                    $('#phoneButton').on('click', function () {
                        ...
                        $("div.signup-phone").show();
                        $("div.signup-mail").remove();
                        $("div.buttons").remove();
                    });
                    $('#emailButton').on('click', function () {
                        ...
                        $("div.password").show();
                        $("div.registration").show();
                        $("div.signup-phone").remove();
                        $("div.signup-mail").show();
                        $("div.buttons").remove();
                    ...
                    $('#submitPhone').on('click', function () {
                        ...
                        postData('/requestContactConfirmation', data, function (result) {
                            ...
                            const urlParams = new URLSearchParams(queryString);
                            var info = '';
                            if (result.error === 'Repeat call is not possible') {
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Для подтверждения телефона воспользуйтесь данными последнего вызова ' +
                                        '(введите последние четыре цифры)';
                                } else {
                                    info += 'Use digits from last flash call to confirn your phone number';
                                }
                                $row.find('.form-input_phone[type="text"]').addClass('form-input_error')
                                    .after('<div class="form-error">' + info + '</div>');
                            } else if (result.error === 'TimeoutMinutesRemainsConfirmationException') {
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Повторно запросить код можно через ' +
                                        Math.floor(result.secondsToWait / 60) + ' мин. и ' +
                                        result.secondsToWait % 60 + ' с.';
                                } else {
                                    info += 'You can request the code again in ' +
                                        Math.floor(result.secondsToWait / 60) + ' min. and ' +
                                        result.secondsToWait % 60 + ' sec.';
                                }
                                $row.find('.form-input_phone[type="text"]').addClass('form-input_error')
                                    .after('<div class="form-error">' + info + '</div>');
                            } else if (result.error === 'AttemptsNumberExceededException') {
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Исчерпано количество попыток ввода кода подтверждения. ' +
                                        'Запросите новый код через ' +
                                        Math.floor(result.secondsToWait / 60) + ' мин. и ' +
                                        result.secondsToWait % 60 + ' с.';
                                } else {
                                    info += 'The number of attempts to enter the confirmation code has been exceeded. ' +
                                        'Request a new code in ' + Math.floor(result.secondsToWait / 60) + ' min. and ' +
                                        result.secondsToWait % 60 + ' sec.';
                                }
                                $row.find('.form-input_phone[type="text"]').addClass('form-input_error')
                                    .after('<div class="form-error">' + info + '</div>');
                            } else if (result.error === 'Re-confirmation prohibited') {
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Повторное подтверждение запрещено';
                                } else {
                                    info += result.error;
                                }
                                function sleep(ms) {
                                ...
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Вам поступит телефонный звонок: введите последние 4 цифры';
                                } else {
                                    info += 'You will receive phone call: please input last four digits';
                                }
                                $this.after('<div class="form-info">' + info + '</div>');
                                ...
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Не удалось отправить подтверждение на указанный контакт';
                                } else {
                                    info += 'Failed to send confirmation to the specified contact';
                                }
                                $row.find('.form-input_phone[type="text"]').addClass('form-input_error')
                                    .after('<div class="form-error">' + info + '</div>');
                            }
                    ...
                    $('#submitPhoneForChange').on('click', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'),
                            data = {token: $row.find('.form-input_code_change').val()};
                        window.location.href = '/confirmDataChange?token=' + data.token;
                    });
                    $('#submitMail').on('click', function () {
                        ...
                        postData('/requestEmailConfirmation', data, function (result) {
                            ...
                            const urlParams = new URLSearchParams(queryString);
                            var info = '';
                            if (result.error === 'No new code generated') {
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'На электронную почту отправлено сообщение с текущим кодом подтверждения';
                                } else {
                                    info += 'A message with the current confirmation code has been sent by the email';
                                }
                                $row.find('.form-input_mail[type="text"]').addClass('form-input_error')
                                    .after('<div class="form-error">' + info + '</div>');
                            } else if (result.error === 'TimeoutMinutesRemainsConfirmationException') {
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Повторно запросить код можно через ' +
                                ...
                                if (urlParams.get('lang') == 'ru') {
                                    info += 'Вам на почту направлен код подтверждения';
                                } else {
                                    info += 'A confirmation code has been sent to you by email';
                                }
                                $this.after('<div class="form-info">' + info + '</div>')
                                $this.attr('disabled', 'disabled');
                                $row.addClass('form-group_confirm');
                    ...
                    $Registration.find('.form-input_code').on('blur', function () {
                        ...
                        if (!$this.hasClass('form-input_error')) {
                            postData('/approveContact', data, function (result) {
                                ...
                                if (result.result) {
                                    if (urlParams.get('lang') == 'ru') {
                                        info += 'Код успешно подтвержден';
                                    } else {
                                        info += 'The code has been successfully confirmed';
                                    }
                                    $this.attr('disabled', 'disabled');
                                    $but.attr('disabled', 'disabled');
                                    $but.after('<div class="form-info">' + info + '</div>');
                                    $this.trigger('change');
                                    $this.trigger('change');
                                } else {
                                    if (result.return) {
                                        ...
                                        if (result.error === 'The confirmation code is outdated. Request a new one') {
                                            if (urlParams.get('lang') == 'ru') {
                                                info += 'Код подтверждения устарел. Запросите новый';
                                            } else {
                                                info += result.error;
                                            }
                                            setTimeout(function (){
                                        ...
                                        } else if (result.error === 'Number of attempts to enter code exceeded') {
                                            if (urlParams.get('lang') == 'ru') {
                                                info += 'Исчерпано количество попыток ввода кода, повторите попытку через 5 минут';
                                            } else {
                                                info += result.error + ', try again in 5 minutes';
                                            }
                                            $row.find('.form-input_code').addClass('form-input_error')
                                                .after('<div class="form-error">' + info + '</div>');
                                        }
                                        ...
                                        $this.addClass('form-input_error').after('<div class="form-error">' + ' ' + '</div>');
                                    } else {
                                        if (urlParams.get('lang') == 'ru') {
                                            info += 'Неверный код';
                                        } else {
                                        ...
                                        $this.addClass('form-input_error').after('<div class="form-error">' + info + '</div>');
                                    }
                                }
                            }, function (result) {
                                if (!result.result) {
                    ...
                    $Registration.find('.form-input').on('change', function () {
                        ...
                        if (($this.hasClass('form-input_phone') || $this.hasClass('form-input_mail')) &&
                            $this.val() && !$this.hasClass('form-input_error')) {
                            $but.prop('disabled', false);
                            $but.next('.form-info').remove();
                            $row.find($regCode).prop('disabled', false);
                        } else {
                    ...
                    $sendButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            if (!error) {
                                postData('/requestContact', $this.data('send'), function (result) {
                                    if (result.result) {
                                        ...
                                        var info = '';
                                        if (urlParams.get('lang') == 'ru') {
                                            info += 'Пользователь не найден или аккаунт не подтверждён';
                                        } else {
                                            info += 'User not found or not approved';
                                        }
                                        $thisStep.find('.form-input:not(.form-input_hide)[type="text"]')
                                            .addClass('form-input_error').after('<div class="form-error">' + info + '</div>');
                                    }
                                })
                            }
                        }
                ...
                init: function () {
                    ...
                    $btnSend.on('click', function (e) {
                        ...
                        if ($this.data('sendstatus') === 'ARCHIVED' && $this.data('check')) {
                            Login().postData('/books/changeBookStatus/', {
                        ...
                        } else if ($this.data('sendstatus') === 'PAID' && $this.data('check')) {
                            Login().postData('/books/changeBookStatus/', {
                                booksIds: $this.data('bookid'),
                                status: 'ARCHIVED'
                            }, function (result) {
                                ...
                                if (result.result) {
                                    ...
                                    if ($cart.length) {
                                        ...
                                        $cart.find('.Cart-product').each(function () {
                                            var $this = $(this);
                                            price += parseInt($this.find('.Cart-price:not(.Cart-price_old)').text().replace(/[^0-9\.]+/g, ""));
                                            priceOld += parseInt($this.find('.Cart-price_old').text().replace(/[^0-9\.]+/g, ""));
                                ...
                                if ($(slick.$slides[slick.$slides.length - 3]).hasClass('slick-active')) {
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        switch ($(slick.$slider).data('load')) {
                                            case'recommended':
                                                sendData('/books/card/recommended')
                                                break;
                                            case'recent':
                                                sendData('/books/card/recent')
                                                break;
                                            case'popular':
                                                sendData('/books/card/popular')
                                                break;
                                        }
                                    }
                                }
                ...
                init: function () {
                    $('form.Topup').on('submit', function (e) {
                        ...
                        Login().postData('/payment', {
                            ...
                            if (result.result) {
                                if (urlParams.get('lang') == 'ru') {
                                    paymentInfo += 'Оплата прошла успешно';
                                } else {
                                    paymentInfo += 'The payment was successful';
                                }
                                var win = window.open(result.data, '_blank');
                                if (win) {
                                    win.focus();
                                    $('.Topup-wrap').append('<div class="form-group">' + '<div class="Topup-success">'
                                        + paymentInfo + '</div>' + '</div>');
                                } else {
                                ...
                                if (urlParams.get('lang') == 'ru') {
                                    paymentInfo += 'Оплата не прошла!';
                                } else {
                                    paymentInfo += 'Payment failed!';
                                }
                                $('.Topup-wrap').append('<div class="form-group">' + '<div class="Topup-fail">'
                                    + paymentInfo + '</div>' + '</div>');
                            }
                        })
                    });
        ...
        var Transactions = function () {
            var transactionTemplate = function (transaction) {
                ...
                const queryString = window.location.search;
                const urlParams = new URLSearchParams(queryString);
                if (transaction.description.indexOf(':') == -1) {
                    if (urlParams.get('lang') == 'ru') {
                        info += 'Пополнение счёта ';
                    } else {
                        info += 'Account replenishment ';
                    }
                } else {
                    var bookMessage = '';
                    if (urlParams.get('lang') == 'ru') {
                        bookMessage += 'Покупка книги ';
                    } else {
                        bookMessage += 'Buying a book ';
                    }
                    var bookName = transaction.description.substring(transaction.description.indexOf(':') + 1);
                    var bookHref = '/books/' + transaction.description.substring(0, transaction.description.indexOf(':'));
                    info += '<span>' + bookMessage + '</span><a href=' + bookHref + '>' + bookName + '</a>'
                }
                return '<tr>\n' + '                      <td>' + transaction.time
